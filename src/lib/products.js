...
//
//  ARRAY ORDER = the sidebar order (and the Tools "All" tab order). Keep it in the
//  sequence the sidebar should read; sndeal is filtered out of the sidebar.
//
//  `icon`  — line icon used in the nav tabs + cards (fallback when no `image`).
//  `image` — real app-icon image (public/) shown on the nav tab, page header + card.
//  `emoji` — fallback used when there's no `image`.
//  `label` — big centered page header + Tools card name.
//  `navLabel` — shorter sidebar tab label.
...
export const PRODUCTS = [
  ...
  {
    // Deal entry — filtered OUT of the sidebar live list; kept for routing/coming-soon copy.
    id: 'sndeal',
