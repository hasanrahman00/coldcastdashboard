...
export default function DomainEnricher() {
  ...
  const stop = async (id) => { try { await api.domainStop(id) } catch (e) { toast(e.message || 'Could not stop', 'err') } }
  const remove = async (job) => {
    const running = job.status === 'running' || job.status === 'queued'
    if (!window.confirm(running ? 'This job is still running. Delete it now?' : 'Delete this job and all its data?')) return
    try { await api.domainDelete(job.id); removeDomainJob(job.id) } catch (e) { toast(e.message || 'Delete failed', 'err') }
  }
    ...
    return (
      <div className="jg" style={{ marginTop: 20 }}>
        <div className="empty">
          ...
          <h3>Domain enricher not connected</h3>
          <p>Set <code>VITE_DOMAIN_SCRAPER_URL</code> to your domain-enricher server, then redeploy the dashboard.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div>
          ...
          slice.map((j) => {
            ...
            return (
              <DomainJobCard
                ...
                downloadUrl={api.domainDownloadUrl(j.id)}
              />
            )
          })
        )}
      ...
      <DomainNewJobModal open={showNew} onClose={() => setShowNew(false)} onCreated={upsertDomainJob} />
      <DomainLogsModal job={logsJob} onClose={() => setLogsJob(null)} />
    </div>
  )
}

function DomainLogsModal({ job, onClose }) {
  ...
  useEffect(() => {
    if (!job) return
    let alive = true
    const load = async () => { try { const r = await api.domainLogs(job.id); if (alive) setLogs((r && r.logs) || []) } catch {} }
    load()
