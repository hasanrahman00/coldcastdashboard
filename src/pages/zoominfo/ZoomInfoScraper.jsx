...
import ZoomInfoJobCard from './ZoomInfoJobCard.jsx'
import ZoomInfoNewJobModal from './ZoomInfoNewJobModal.jsx'
...
export default function ZoomInfoScraper() {
  ...
  const run = async (id) => {
    try { await api.zoominfoStart(id) }
    catch (e) {
  ...
  const stop = async (id) => { try { await api.zoominfoStop(id) } catch (e) { toast(e.message || 'Could not stop', 'err') } }
  const remove = async (job) => {
    const running = job.status === 'running' || job.status === 'queued'
    if (!window.confirm(running ? 'This job is still running. Delete it now?' : 'Delete this job and all its data?')) return
    try { await api.zoominfoDelete(job.id); removeZoomInfoJob(job.id) } catch (e) { toast(e.message || 'Delete failed', 'err') }
  }
    ...
    return (
      <div className="jg" style={{ marginTop: 20 }}>
        <div className="empty">
          ...
          <h3>ZoomInfo server not connected</h3>
          <p>Set <code>VITE_ZOOMINFO_SCRAPER_URL</code> to your ZoomInfo scraper server, then redeploy the dashboard.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div>
      <ScraperConnection scraper="zoominfo" name="ZoomInfo scraping" />
          ...
          slice.map((j) => {
            ...
            return (
              <ZoomInfoJobCard
                ...
                downloadUrl={api.zoominfoDownloadUrl(j.id, 'csv')}
                xlsxUrl={api.zoominfoDownloadUrl(j.id, 'xlsx')}
              />
            )
          })
        )}
      ...
      <ZoomInfoNewJobModal open={showNew} onClose={() => setShowNew(false)} onCreated={upsertZoomInfoJob} />
      <ZoomInfoLogsModal job={logsJob} onClose={() => setLogsJob(null)} />
    </div>
  )
}

function ZoomInfoLogsModal({ job, onClose }) {
  ...
  useEffect(() => {
    if (!job) return
    let alive = true
    const load = async () => { try { const r = await api.zoominfoLogs(job.id); if (alive) setLogs((r && r.logs) || []) } catch {} }
    load()
