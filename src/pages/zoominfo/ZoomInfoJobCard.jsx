...
// Reuses the shared .jc* card CSS. ZoomInfo people/company rows; CSV + XLSX export.
export default function ZoomInfoJobCard({ job, anotherRunning, onRun, onStop, onDelete, onOpenLogs, downloadUrl, xlsxUrl }) {
  ...
  const kind = job.searchType === 'company' ? 'companies' : 'people'
  ...
  return (
    <div className={'jc jc-' + stCls}>
      ...
      <div className="jc-a">
        ...
        <button className="btn btn-xlsx btn-sm" onClick={() => window.open(xlsxUrl, '_blank')} disabled={!hasData}><IconDownload /> XLSX</button>
