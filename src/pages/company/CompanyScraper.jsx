...
// Company runs on its OWN server. Its jobs are streamed once, globally, by the shared
// AppStore (so the header's Total/Running counters stay live from any tab); this tab
// just renders that shared list + owns the Company-specific UI (grid, paging, modals).
export default function CompanyScraper() {
  ...
  const remove = async (job) => {
    ...
    try {
      await api.companyDelete(job.id)
      removeCompanyJob(job.id)
    } catch (e) {
    ...
    return (
      <div className="jg" style={{ marginTop: 20 }}>
        <div className="empty">
          ...
          <h3>Company server not connected</h3>
          <p>
            Set <code>VITE_COMPANY_SCRAPER_URL</code> to your Company scraper server, then redeploy the dashboard.
          </p>
        </div>
      </div>
    )
  ...
  return (
    <div>
          ...
          slice.map((j) => {
            ...
            return (
              <CompanyJobCard
                ...
                downloadUrl={api.companyDownloadUrl(j.id)}
              />
            )
          })
        )}
      ...
      <CompanyNewJobModal open={showNew} onClose={() => setShowNew(false)} onCreated={upsertCompanyJob} />
      <CompanyLogsModal job={logsJob} onClose={() => setLogsJob(null)} />
    </div>
  )
}

function CompanyLogsModal({ job, onClose }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const r = await api.companyLogs(job.id)
