...
export default function StatsBox() {
  const { jobs, apolloJobs, apolloFreeJobs, enricherJobs, companyJobs, enrichUploads } = useApp()

  // Jobs + Running span ALL products: Sales Nav + Apollo (+ Apollo Free) + Company scrape
  // jobs, LinkedIn URL enricher jobs, and Waterfall enrich uploads.
  const enrichJobs = Object.values(enrichUploads || {})
  const apollo = apolloJobs || []
  const apolloFree = apolloFreeJobs || []
  const linkedin = enricherJobs || []
  const company = companyJobs || []
  const totalJobs = jobs.length + apollo.length + apolloFree.length + company.length + linkedin.length + enrichJobs.length
  const running =
    jobs.filter((j) => j.status === 'running').length +
    apollo.filter((j) => j.status === 'running' || j.status === 'stopping').length +
    apolloFree.filter((j) => j.status === 'running' || j.status === 'stopping').length +
